#!/bin/bash
# ─────────────────────────────────────────────────────────────
# WellNest — DEV Environment Deployment Script
# Environment: dev
# Namespace: wellnest-dev
# Phase: 2 — Kubernetes Manifests
# ─────────────────────────────────────────────────────────────
# Usage: chmod +x apply-dev.sh && ./apply-dev.sh
# ─────────────────────────────────────────────────────────────
set -e
...
echo ""
echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     WellNest — DEV Deployment Script     ║${NC}"
echo -e "${BLUE}║     Namespace: wellnest-dev              ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
...
kubectl apply -f "$K8S_DIR/configmap/configmap-dev.yaml"
...
kubectl apply -f "$K8S_DIR/secrets/sealedsecret-dev.yaml"
...
kubectl apply -f "$K8S_DIR/mongodb/statefulset-dev.yaml"
kubectl apply -f "$K8S_DIR/mongodb/service-dev.yaml"
...
kubectl apply -f "$K8S_DIR/mongodb/rs-init-job-dev.yaml"
...
kubectl apply -f "$K8S_DIR/backend-services/auth-deployment-dev.yaml"
kubectl apply -f "$K8S_DIR/backend-services/assessment-deployment-dev.yaml"
kubectl apply -f "$K8S_DIR/backend-services/therapist-deployment-dev.yaml"
log_ok "Backend deployments applied"

log_step "Step 10: Frontend Rollout"
kubectl apply -f "$K8S_DIR/frontend/frontend-rollout-dev.yaml"
log_ok "Frontend Rollout applied"

log_step "Step 11: Services"
kubectl apply -f "$K8S_DIR/services/services-dev.yaml"
log_ok "Services applied"

log_step "Step 12: HPA"
kubectl apply -f "$K8S_DIR/hpa/frontend-hpa-dev.yaml"
log_ok "HPA applied"

log_step "Step 13: Network Policies"
kubectl apply -f "$K8S_DIR/network-policy/networkpolicy-dev.yaml"
log_ok "NetworkPolicies applied"

log_step "Step 14: KGateway"
kubectl apply -f "$K8S_DIR/gateway/kgateway-dev.yaml"
...
echo ""
echo "Access:"
echo "  Add to /etc/hosts: <NODE_IP>  wellnest-dev.local"
echo "  Open: http://wellnest-dev.local"
echo ""
